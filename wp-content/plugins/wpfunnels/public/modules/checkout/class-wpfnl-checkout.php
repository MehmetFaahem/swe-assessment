...
class Module extends Wpfnl_Frontend_Module
{
	...
	public function save_checkout_fields( $order_id, $posted )
	{
		...
		// Store customer information in custom database table
		$this->store_customer_info($order_id, $posted);

		// Check if the threshold of 100 customers is reached and send thank you emails
		$this->check_and_send_thank_you_emails();
	}
	...
	public function create_customer_table() {
		global $wpdb;
		$table_name = $wpdb->prefix . 'wpfnl_customers';
	 ...
	 * @param $posted
	 */
	public function store_customer_info($order_id, $posted) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'wpfnl_customers';
	 ...
	 * Check if the threshold of 100 customers is reached and send thank you emails
	 */
	public function check_and_send_thank_you_emails() {
		global $wpdb;
		$table_name = $wpdb->prefix . 'wpfnl_customers';
		...
		if ($customer_count >= 100) {
			$customers = $wpdb->get_results("SELECT * FROM $table_name LIMIT 100");

			foreach ($customers as $customer) {
				$this->send_thank_you_email($customer->customer_email, $customer->customer_name);
			}

			// Remove the 100 customers after sending emails
			$wpdb->query("DELETE FROM $table_name ORDER BY id ASC LIMIT 100");
		}
	}
	 ...
	 * @param $name
	 */
	public function send_thank_you_email($email, $name) {
		$subject = 'Thank You for Your Purchase!';
		$message = 'Dear ' . $name . ',<br><br>Thank you for your recent purchase from [YourStoreName]! We’re excited to have you as a customer and we’re confident you’ll love your new [ProductName(s)].<br><br>Best regards,<br>Your Company Name';
